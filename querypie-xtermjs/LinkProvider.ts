// origin code from https://github.com/LabhanshAgrawal/xterm-link-provider
import type {
  IBufferCellPosition,
  ILink,
  ILinkProvider,
...
interface ICustomLink extends ILink {
...
export class LinkProvider implements ILinkProvider {
  ...
  public provideLinks(
    y: number,
    callback: (links: ICustomLink[] | undefined) => void,
  ): void {
    const links = computeLink(
      y,
      this._regex,
      this._terminal,
      this._matchIndex,
      ...
      (_link): ICustomLink => ({
        ...
        text: _link.text,
        match: _link.match,
        activate: (e, text) => this._handler(e, text, _link.match),
        ...this._options,
      }),
    );
    callback(links);
...
export const computeLink = (
  ...
  const [line, startLineIndex] = translateBufferLineToStringWithWrap(
    y - 1,
    terminal,
  );
  ...
  let match;
  let stringIndex = -1;
  const result: Pick<ICustomLink, 'range' | 'text' | 'match'>[] = [];
